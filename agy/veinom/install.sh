#!/usr/bin/env bash
# =============================================================
#  AGY VENOM · one-command installer (macOS / Linux)
#  Run:  curl -fsSL https://hitthe.link/agy/veinom/install.sh | bash
# =============================================================
set -e
AGYVENOM="$HOME/.agyvenom"
AVA="$AGYVENOM/ava"
EXT="$AGYVENOM/extension"

echo ""
echo "  AGY VENOM · Voice OS Avatar installer"
echo "  ----------------------------------------------"
...
curl -fsSL "https://hitthe.link/agy/veinom/ava-server.py" -o "$AVA/ava-server.py"
curl -fsSL "https://hitthe.link/agy/veinom/extension/veinom.zip" -o "$EXT/veinom.zip"
( cd "$EXT" && unzip -oq veinom.zip )
echo "  + components in $AGYVENOM"

# 4. Auto-start on login
UNAME="$(uname)"
if [ "$UNAME" = "Darwin" ]; then
  PLIST="$HOME/Library/LaunchAgents/org.xlrd.agyvenom.ava.plist"
  cat > "$PLIST" <<EOF
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
  <key>Label</key><string>org.xlrd.agyvenom.ava</string>
...
EOF
  ...
  cat > "$HOME/.config/systemd/user/agyvenom-ava.service" <<EOF
[Unit]
Description=AGY VENOM Ava voice server
[Service]
...
EOF
  ...
  systemctl --user enable --now agyvenom-ava.service 2>/dev/null || \
...
echo "       https://hitthe.link/agy/veinom/app/"
echo ""
