# =============================================================
#  AGY VENOM · one-command installer (Windows / PowerShell)
#  Run:  irm https://hitthe.link/agy/veinom/install.ps1 | iex
# =============================================================
$ErrorActionPreference = "Stop"
$AGYVENOM = "$env:USERPROFILE\.agyvenom"
$EXT    = "$AGYVENOM\extension"
$AVA    = "$AGYVENOM\ava"

Write-Host ""
Write-Host "  AGY VENOM  ·  Voice OS Avatar installer" -ForegroundColor Green
...
Invoke-WebRequest "https://hitthe.link/agy/veinom/ava-server.py" -OutFile "$AVA\ava-server.py" -UseBasicParsing
Invoke-WebRequest "https://hitthe.link/agy/veinom/extension/veinom.zip" -OutFile "$EXT\veinom.zip" -UseBasicParsing
Expand-Archive -Path "$EXT\veinom.zip" -DestinationPath $EXT -Force
Write-Host "  + components downloaded to $AGYVENOM"

# 4. Auto-start the Ava server on login (HKCU Run + Startup shortcut)
$run = "`"$py`" `"$AVA\ava-server.py`""
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run" -Name "AgyVenomAva" -Value $run -ErrorAction SilentlyContinue
...
Write-Host "  4. Visit any AI chat + click Start on the AGY VENOM panel"
Write-Host ""
Write-Host "  Or skip the extension entirely — open the web app:" -ForegroundColor DarkGreen
Write-Host "       https://hitthe.link/agy/veinom/app/" -ForegroundColor Cyan
Write-Host ""
Start-Process "chrome://extensions/"
Start-Process "https://hitthe.link/agy/veinom/app/"
